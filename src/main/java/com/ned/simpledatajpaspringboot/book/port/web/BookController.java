...
public class BookController {
    ...
    public ResponseEntity<BookControllerResult> getAllBoook(@RequestParam(value = "bookname", required = false) String bookname) {
        if (bookname == null || bookname == "") {
            ...
            if (!foundBookDtoOpt.isPresent()) return ResponseEntity.ok().body(new BookControllerResult(true));
    ...
    public ResponseEntity<BookControllerResult> modifyBookName(@PathVariable("id") long id, @RequestBody BookDto bookDto) {
        if (id == 0) return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new BookControllerResult(false, "Invalid id=" + id));
        String bookName = bookDto.getName();
        if (bookName == null || bookName == "") return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new BookControllerResult(false, "Invalid bookName=" + bookName));
